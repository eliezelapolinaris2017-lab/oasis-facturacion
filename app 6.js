...
async function makePDF() {
  ...
  const taxRate = Number(doc.taxRate ?? cfg.taxRate ?? 11.5);
  ...
  // Encabezado estilo clásico anterior
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(19);
  pdf.text(docTitle, marginX, 50);

  pdf.setFontSize(10);
  pdf.text(`No.: ${docNumber}`, marginX, 72);
  pdf.text(`Fecha: ${safeText(doc.date || today())}`, marginX, 88);

  let y = 114;
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(11);
  pdf.text(safeText(cfg.biz?.name || "Oasis Air Cleaner Services LLC"), marginX, y);
  y += 15;

  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(10);
  if (cfg.biz?.addr) {
    y = writeWrapped(cfg.biz.addr, marginX, y, 420, 12);
    y += 4;
  }
  if (cfg.biz?.phone) {
    pdf.text(`Tel: ${safeText(cfg.biz.phone)}`, marginX, y);
    y += 15;
  }
  if (cfg.biz?.email) {
    pdf.text(`Email: ${safeText(cfg.biz.email)}`, marginX, y);
    y += 15;
  }
  ...
  pdf.setFontSize(10);
  if (doc.client?.name) {
    y = writeWrapped(doc.client.name, marginX, y, 460, 12);
  }
  if (doc.client?.contact) {
    y = writeWrapped(doc.client.contact, marginX, y, 460, 12);
  }
  if (doc.client?.addr) {
    y = writeWrapped(doc.client.addr, marginX, y, 460, 12);
  }

  y += 12;
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(11);
  pdf.text("Válida hasta", marginX, y);
  y += 15;
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(10);
  pdf.text(safeText(validUntil), marginX, y);
  y += 22;
  ...
  pdf.text(money(sub), valueX, y, { align: "right" });
  y += 16;

  pdf.text(`IVU (${taxRate.toFixed(2)}%):`, labelX, y);
  pdf.text(money(tax), valueX, y, { align: "right" });
  y += 18;

  pdf.setFontSize(12);
  pdf.text("TOTAL:", labelX, y);
  pdf.text(money(grand), valueX, y, { align: "right" });
  y += 34;

  const ensureRoom = (needed = 80) => {
    if (y + needed > pdf.internal.pageSize.getHeight() - 65) {
      pdf.addPage();
      y = 50;
    }
  };
  ...
  if (doc.notes) {
    ...
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(10);
    y = writeWrapped(doc.notes, marginX, y, pageW - marginX * 2, 12) + 14;
  }
  ...
  // Footer clásico
  const footerY = pdf.internal.pageSize.getHeight() - 32;
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(9);
  pdf.setTextColor(80, 80, 80);
  pdf.text(`${safeText(cfg.biz?.name || "Oasis Air Cleaner Services LLC")} · ${docTitle} ${docNumber}`, pageW / 2, footerY, { align: "center" });

/* =========================
   OASIS / NEXUS INVOICING — app.js (FULL) ✅ PRO
   - GitHub Pages friendly
   - Historial REAL (tabla + abrir + PDF)
   - Customers + Vendors + Reporting + Config
   - PDF (jsPDF + AutoTable) con FIX:
       ✅ Logo NO pisa texto
       ✅ No. y Fecha NO se montan
========================= */
...
(function ensureHubFab(){
  ...
  a.className = "hub-fab"; // usa tu CSS si existe
...
/* ====== ITEMS RENDER (NO re-render por tecla) ====== */
...
function renderHistory() {
  const db = loadDB();
  const body = $("histBody");
  if (!body) return; // por si alguien rompe el HTML
...
function buildPdfDoc() {
  ...
  // FIX #2: No. y Fecha separados (NO se montan)
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.text(`No.: ${state.current.number || "AUTO"}`, margin, 86);
  doc.text(`Fecha: ${state.current.date || ""}`, margin, 102);

  // FIX #1: Business right + logo sin overlap
  const rightX = W - margin;

  // Logo arriba a la derecha (más pequeño) y texto debajo del logo
  let textTopY = 52;
  if (biz.logoDataUrl) {
    try {
      ...
      // Empuja el texto debajo del logo
...
function renderVendors() {
  ...
  // Si estaba "en construcción", lo vuelve funcional sin romper nada
...
function bindEvents() {
  ...
  $("invoiceSubtabs").addEventListener("click", (e) => {
    ...
    if (b.dataset.sub === "preview") makePreview();
  });

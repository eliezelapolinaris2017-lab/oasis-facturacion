/* =========================
   OASIS / NEXUS INVOICING — app.js (FULL) ✅ FIREBASE PRO + LOCAL BACKUP
========================= */
...
function refreshAuthUI() {
  ...
  ["btnSaveDoc", "btnPDF", "btnConfirmFromPreview", "btnExportHist", "btnClearHist", "btnAddCustomer", "btnExportBackup", "btnRestoreBackup"]
...
async function buildBackupPayload() {
  return {
    ...
    version: "nexus_invoicing_backup_local_v1",
...
async function exportBackupFile() {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `nexus_invoicing_backup_${toISODate(new Date())}.json`;
  a.click();
...
async function restoreBackupFromFile(file) {
  ...
  if (!parsed || parsed.version !== "nexus_invoicing_backup_local_v1") {
...
function refreshCatalogSelects({ keepSelection } = { keepSelection: false }) {
  ...
  if (svcSelect) {
    ...
    empty.value = "";
    empty.textContent = "Selecciona servicio";
    svcSelect.appendChild(empty);
...
function bindEvents() {
  ...
  $("restoreBackupFile")?.addEventListener("change", async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert(err?.message || "No se pudo restaurar el backup.");
    }

...
function normalizeCfg(c) {
  const d = defaultCfg();
  return { ...d, ...(c || {}), biz: { ...d.biz, ...(c?.biz || {}) }, taxRate: Number(c?.taxRate ?? 11.5) };
}
...
function readForm() {
  ...
  c.taxRate = Number(state.cfg?.taxRate ?? 11.5);
...
function calcTotals() {
  ...
  const tax = sub * (Number(state.current.taxRate ?? state.cfg?.taxRate ?? 11.5) / 100);
...
async function makePDF() {
  ...
  let y = 42;
  if (cfg.biz.logoDataUrl) { try { pdf.addImage(cfg.biz.logoDataUrl, "PNG", 42, 32, 58, 58); } catch {} }
  pdf.setFont("helvetica", "bold"); pdf.setFontSize(18); pdf.text(cfg.biz.name || "Nexus Invoicing", 115, y);
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(9); pdf.text([cfg.biz.phone, cfg.biz.email, cfg.biz.addr].filter(Boolean).join(" · "), 115, y + 16);
  pdf.setFont("helvetica", "bold"); pdf.setFontSize(16); pdf.text(state.current.type === "FAC" ? "FACTURA" : "COTIZACIÓN", 430, y); pdf.setFontSize(10); pdf.text(state.current.number || "", 430, y + 18);
  y = 122;
  pdf.setFont("helvetica", "bold"); pdf.text("Cliente", 42, y); pdf.setFont("helvetica", "normal"); pdf.text(state.current.client.name || "", 42, y + 16); pdf.text(state.current.client.contact || "", 42, y + 31); pdf.text(state.current.client.addr || "", 42, y + 46); pdf.text(`Fecha: ${state.current.date}`, 430, y + 16); pdf.text(`Estado: ${state.current.status}`, 430, y + 31);
  const rows = (state.current.items || []).map(it => [it.desc, Number(it.qty || 0).toFixed(2), money(it.price), money(Number(it.qty || 0) * Number(it.price || 0))]);
  pdf.autoTable({ startY: 195, head: [["Descripción", "Cant.", "Precio", "Total"]], body: rows, theme: "grid", styles: { fontSize: 9, cellPadding: 7 }, headStyles: { fillColor: [18, 18, 28] } });
  y = pdf.lastAutoTable.finalY + 18;
  pdf.setFont("helvetica", "bold"); pdf.text(`Subtotal: ${money(state.current.totals.sub)}`, 390, y); pdf.text(`IVU: ${money(state.current.totals.tax)}`, 390, y + 16); pdf.text(`Total: ${money(state.current.totals.grand)}`, 390, y + 32);
  y += 64; pdf.setFont("helvetica", "normal"); pdf.setFontSize(9);
  if (state.current.notes) { pdf.setFont("helvetica", "bold"); pdf.text("Notas", 42, y); pdf.setFont("helvetica", "normal"); pdf.text(pdf.splitTextToSize(state.current.notes, 500), 42, y + 14); y += 50; }
  if (state.current.terms) { pdf.setFont("helvetica", "bold"); pdf.text("Condiciones", 42, y); pdf.setFont("helvetica", "normal"); pdf.text(pdf.splitTextToSize(state.current.terms, 500), 42, y + 14); }
  pdf.save(`${state.current.number || "documento"}.pdf`);
}
...
function refreshSettingsUI() {
  ...
  if ($("taxRate")) $("taxRate").value = state.cfg.taxRate || 11.5;
}
